"use strict";
function calculateChargingSchedule(params) {
...
function updateResults(result) {
    ...
    if (!result) {
        ...
        if (durationEl)
            durationEl.textContent = '-- hrs -- min';
        if (energyRequiredEl)
            energyRequiredEl.textContent = '-- kWh';
        if (chargingPowerEl)
            chargingPowerEl.textContent = '-- kW';
        if (effectiveRateEl)
            effectiveRateEl.textContent = '-- kW';
        if (summaryTextEl) {
    ...
    if (energyRequiredEl)
        energyRequiredEl.textContent = `${result.energyRequired} kWh`;
    if (chargingPowerEl)
        chargingPowerEl.textContent = `${result.chargingPower} kW`;
    if (effectiveRateEl)
        effectiveRateEl.textContent = `${result.effectiveRate} kW`;
    if (summaryTextEl) {
